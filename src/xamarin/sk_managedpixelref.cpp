...
sk_pixel_ref_t* sk_managedpixelref_new_from_existing(void* context, void* pixelRef) {
    return ToSkPixelRef(new SkManagedPixelRef(context, (SkPixelRef*)pixelRef));
}

sk_pixel_ref_t* sk_managedpixelref_new(
...
void sk_managedpixelref_delete(sk_pixel_ref_t* d) {
    delete AsSkManagedPixelRef(d);
}

sk_isize_t sk_managedpixelref_dimensions(sk_pixel_ref_t* d) {
    return ToISize(AsSkManagedPixelRef(d)->pixelRef->dimensions());
}
int32_t sk_managedpixelref_width(sk_pixel_ref_t* d) {
    return AsSkManagedPixelRef(d)->pixelRef->width();
}

int32_t sk_managedpixelref_height(sk_pixel_ref_t* d) {
    return AsSkManagedPixelRef(d)->pixelRef->height();
}
size_t sk_managedpixelref_rowBytes(sk_pixel_ref_t* d) {
    return AsSkManagedPixelRef(d)->pixelRef->rowBytes();
}
void* sk_managedpixelref_pixels(sk_pixel_ref_t* d) {
    return AsSkManagedPixelRef(d)->pixelRef->pixels();
}
void* sk_managedpixelref_pixel_ref(sk_pixel_ref_t* d) {
    // IMPORTANT!!!
...
uint32_t sk_managedpixelref_generation_id(sk_pixel_ref_t* d) {
    return AsSkManagedPixelRef(d)->pixelRef->getGenerationID();
}
void sk_managedpixelref_notify_pixels_changed(sk_pixel_ref_t* d) {
    AsSkManagedPixelRef(d)->pixelRef->notifyPixelsChanged();
}
bool sk_managedpixelref_is_immutable(sk_pixel_ref_t* d) {
    return AsSkManagedPixelRef(d)->pixelRef->isImmutable();
}
void sk_managedpixelref_set_immutable(sk_pixel_ref_t* d) {
    AsSkManagedPixelRef(d)->pixelRef->setImmutable();
}
void sk_managedpixelref_add_generation_id_listener(sk_pixel_ref_t* d, sk_idchangelistener_t* listener) {
    AsSkManagedPixelRef(d)->pixelRef->addGenIDChangeListener(sk_ref_sp(AsSkManagedIDChangeListener(listener)));
}
void sk_managedpixelref_notify_added_to_cache(sk_pixel_ref_t* d) {
    AsSkManagedPixelRef(d)->pixelRef->notifyAddedToCache();
}

void sk_managedpixelref_set_procs(sk_pixel_ref_procs_t procs) {
    gProcs = procs;
